...
def preprocess_fn(img_id, image, box, is_training):
    ...
    def resizer_train(image, box, min_side, max_side):
    ...
    def resizer_val(image, min_side, max_side):
        ...
        return padded_imgs.astype(np.float32), scale
    ...
    def _infer_data(image, input_shape):
        ...
        return img_id, image, np.array((img_h, img_w), np.float32), scale

    def _data_aug(image, box, is_training, image_size=(600, 600)):
        """Data augmentation function."""
        ih, iw, _ = image.shape # 480 640
        w, h = image_size # 600 600
...
def create_retinanet_dataset(mindrecord_file, batch_size, repeat_num, device_num=1, rank=0,
    ...
    if is_training:
        output_columns = ["image", "box", "box_weights", "label", "bucket", "bucket_weights", "num_match"]
        trans_before_resize = [color_adjust_op, normalize_op]
        trans_after_resize = [change_swap_op]
    else:
        output_columns = ["img_id", "image", "image_shape", "scale"]
    ...
    ds = ds.map(operations=trans_after_resize, input_columns=["image"], python_multiprocessing=is_training,
                num_parallel_workers=num_parallel_workers)
    ...
    # # image j[0].shape:Tensor(1, 3, 600, 600) 1是batch_size 
    # # box j[1].shape:Tensor(1, 7555, 4)
    # # label j[2].shape:Tensor(1, 7555)
    # # bucket j[3].shape:Tensor(1, 7555, 32)
    # # num_matches j[4]:Tensor(shape=[1, 1], dtype=Int32, value=[[21]])
